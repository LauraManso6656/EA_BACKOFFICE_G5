...
@Component({
  ...
  template: `
    <div *ngIf="sessionService.isVisible()" 
         class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
         [@backdropAnimate]>
      
      <div class="bg-primary-container border border-outline-variant/30 rounded-3xl p-8 max-w-md w-full shadow-2xl"
           [@modalAnimate]>
        
        <div class="flex flex-col items-center text-center">
          <div class="w-20 h-20 bg-secondary/10 rounded-full flex items-center justify-center mb-6">
            <span class="material-symbols-outlined text-4xl text-secondary animate-bounce">timer</span>
          </div>
          
          <h2 class="text-2xl font-bold text-white mb-2">Session Expiring</h2>
          <p class="text-on-surface-variant mb-8">
            Your secure session is about to expire for security reasons. Would you like to stay connected or logout now?
          </p>
          
          <div class="flex flex-col w-full gap-3">
            <button (click)="sessionService.extendSession()"
            ...
            <button (click)="sessionService.logout()"
